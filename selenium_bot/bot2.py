    ...
    def fill_certificate(self):
        try:
            ...
            if self.current_index==0:
                ...
                # ---------- OPTIONAL: try to switch page length to 100 if the control exists ----------
                try:
                    length_select = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "#authImportItmeTbl_length select")))
                    self.driver.execute_script("arguments[0].value='100'; arguments[0].dispatchEvent(new Event('change'));", length_select)
                    # Give DataTable a moment to reload after changing length
                    time.sleep(1.2)
                except Exception:
                    pass  # If no selector, continue with default pagination
                
                # ---------- Extract table across all pages ----------
                all_rows = []
                
                while True:
                    # Wait for tbody to be present
                    tbody = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "#authImportItmeTbl tbody")))
                    rows = tbody.find_elements(By.TAG_NAME, "tr")
                
                    # Handle "No data available" guard
                    if len(rows) == 1:
                        only_td = rows[0].find_elements(By.TAG_NAME, "td")
                        if only_td and "No data available" in only_td[0].text:
                    ...
                    for r in rows:
                        ...
                        if not tds:
                            continue
                        page_data.append([c.text.strip() for c in tds])
                
                    # Append page data
                    if page_data:
                        all_rows.extend(page_data)
                
                    # Find and handle Next button
                    try:
                        next_btn = self.driver.find_element(By.ID, "authImportItmeTbl_next")
                        classes = next_btn.get_attribute("class") or ""
                        if "disabled" in classes:
                            print("✅ No more pages.")
                            break
                
                        # Keep a reference to current tbody to wait for staleness (ensures new page loaded)
                        old_tbody = tbody
                        self.driver.execute_script("arguments[0].scrollIntoView({block:'center'});", next_btn)
                        next_btn.click()
                
                        # Wait for the old tbody to become stale (prevents duplicate reads / race)
                        WebDriverWait(self.driver, 15).until(EC.staleness_of(old_tbody))
                        # Small buffer
                        time.sleep(0.5)
                ...
                # ---------- Build header ----------
                # Note: sometimes headers have blanks; we normalize and ensure unique column names
                raw_headers = [h.text.strip() for h in self.driver.find_elements(By.CSS_SELECTOR, "#authImportItmeTbl thead th")]
                headers = []
                seen = {}
                for h in raw_headers:
                    key = h if h else "Column"
                    # ensure uniqueness (Column, Column_2, ...)
                    if key in seen:
                        seen[key] += 1
                ...
                # If there are rows but header count mismatches, align by trimming/padding
                if all_rows:
                    max_cols = max(len(r) for r in all_rows)
                    if len(headers) < max_cols:
                        # pad headers
                        pad = [f"Column_{i}" for i in range(len(headers)+1, max_cols+1)]
                        headers = headers + pad
                ...
                df = pd.DataFrame(normalized, columns=headers) if normalized else pd.DataFrame(columns=headers)
                
                # ---------- Save to Downloads with timestamp ----------
                downloads_path = os.path.join(os.path.expanduser("~"), "Downloads")
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = f"EPCG_import_items_{timestamp}.xlsx"
                file_path = os.path.join(downloads_path, filename)
                
                print("DOWNLOADS PATH:", downloads_path)
                print("DOWNLOADS EXISTS:", os.path.exists(downloads_path))
                print("FILE PATH:", file_path)
                print("DF SHAPE:", df.shape)
                
                # Try saving; on failure, fallback to current working directory
                try:
                    # engine='openpyxl' ensures modern .xlsx writing
                    df.to_excel(file_path, index=False, engine="openpyxl")
                    assert os.path.exists(file_path), "File not found after save attempt."
                    print(f"✅ Excel Saved Successfully: {file_path}")
                except Exception as e:
                    print("❌ ERROR SAVING EXCEL:", e)
                    try:
                        fallback = os.path.abspath(f"EPCG_import_items_{timestamp}.xlsx")
                        df.to_excel(fallback, index=False, engine="openpyxl")
                        print(f"✅ Saved to project folder instead: {fallback}")
                    except Exception as e2:
                        print("❌ SECONDARY SAVE FAILED:", e2)
